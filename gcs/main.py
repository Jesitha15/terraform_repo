        ...
        for cell in cells:
            try:
                ...
                value = (
                    ...
                    else [cell["value"]] if not isinstance(cell["value"], list)
                    else cell["value"]
                )

                if value and isinstance(value, list) and value[0]:
                    try:
                        parsed_data = json.loads(value[0])  # Parse JSON string
                    except:
            ...
            # Yield rows with the SAME timestamps for this record
...
class SelectLatestTimestampByColumn(beam.DoFn):
    def process(self, element):
        # Element is a tuple: ((row_key, column), [records])
        (row_key, column), records = element
        if not records:
            return

        # Select the record with the latest timestamp
        latest_record = max(
            records,
